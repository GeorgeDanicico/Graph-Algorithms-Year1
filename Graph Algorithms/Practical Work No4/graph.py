...
# from Bellman_Ford import BellmanFordAlgortihm
...
class DirectedGraph:
    ...
    def getAllEdges(self):
        for edge in self.__dictCost.keys():
            yield edge
   ...
   #HERE     # Topological sort => each vertex v, if there is an edge (v, u), v must appear before you
    ...
    def BellmanFordAlgorithm(self, v1, v2):
        """
        This function will return the minimum cost of a walk from vertex v1 to vertex v2
        :param v1: the source vertex
        :param v2: the destination vertex
        :return: the minimum cost if it exists
        """

        if not self.vertexExistence(v1) or not self.vertexExistence(v2):
            raise Exception("Invalid vertices")

        # return BellmanFordAlgortihm(self, v1, v2)